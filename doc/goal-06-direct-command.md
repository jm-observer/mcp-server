...
- **【高危预警】这只是路径规范化层面的约束，并非真正的执行安全隔离！** 任意执行者都可以下发带有绝对路径的毁灭性指令（诸如执行 `rm -rf /`，或者是 `cat ~/.ssh/id_rsa` 等），仅仅锁定工作目录根本拦不住此类底层系统级的越权操作。强行开启 `allow_direct_command = true` 等同于交出了当前服务器用户的**完全远程控制权 (RCE)**，此项功能只能严格限制在受受信和严格隔离的沙箱（如单独封装的 Docker）环境中开放使用。
