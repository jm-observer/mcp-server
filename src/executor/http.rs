...
impl HttpExecutor {
            ...
            if c == '$' && chars.peek() == Some(&'{') {
                ...
                if let Some(val) = args.get(&var_name) {
                    match val {
                        ...
                        _ => {
                            return Err(HttpError::TemplateResolution(format!(
                                "Variable {} is not simple",
                                var_name
                            )));
                        }
                    }
    ...
    pub async fn execute(
        ...
        for (k, v) in &tool.env {
            let h_name = HeaderName::from_bytes(k.as_bytes()).map_err(|_| HttpError::InvalidHeader(k.clone()))?;
            let h_val = HeaderValue::from_str(v).map_err(|_| HttpError::InvalidHeader(v.clone()))?;
